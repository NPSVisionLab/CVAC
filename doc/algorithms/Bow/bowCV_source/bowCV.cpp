...
bool bowCV::train_parseTrainList(const string& _filepathTrain,const string& _filenameTrainList)
{
	...
	do
	{
		...
		train_stackTrainImage(_fullFilePathImg,_classID);
...
// 		if(vClassIDTrain[k]=="2")	//lekomin (test)
// 			continue;
...
{	
	...
	if(!flagTrain)
	{
		cout << "Error - before testing, train data is necessary .. "<<endl;	fflush(stdout);
		return false;
	}	
	...
	if(_img.empty())
	{
		cout << "Error - no file: " << _fullfilename <<endl;	fflush(stdout);
		return false;
	}
