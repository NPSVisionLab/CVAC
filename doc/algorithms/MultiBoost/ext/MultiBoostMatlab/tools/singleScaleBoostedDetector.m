...
while min(D(:))<10  % This number is arbitrary. However represents the distance threshold of close pixels. It is clear that this number depends on the scale used.
    N = length(x);
    [i,j] = find(D==min(D(:)));
