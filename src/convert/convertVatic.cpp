...
LabelableList* cvac::convert(std::string filePath) {
	...
	while(std::getline(inputFile, inputLine)) {
		...
		LabeledTrack* curTrack;    // Insert for new Object Id
		if(objectTracks.end() == objectTracks.find(trackObjId)) {
		...
		if(NULL == curTrack) {
			throw new exception("LabeledTrack map did not contain key for given 'Track_obj_ID'");
		}
